...
def get_reported_articles(db: Session = Depends(get_db)):
...
def hide_category(category_id: int, db: Session = Depends(get_db)):
...
def add_blacklist_keyword(data: BlacklistKeywordRequest , db: Session = Depends(get_db)):
    keyword = data.keyword.strip().lower()
    existing = db.query(BlacklistedKeyword).filter_by(keyword=keyword).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"'{keyword}' is already blacklisted.")
